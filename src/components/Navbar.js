  ...
  return (
    <div>
      <div className="flex justify-between items-center w-full h-20 text-white bg-black px-4 fixed">
        <div>
          <h1 className="text-3xl ml-2">Adarsh</h1>
        </div>
        <ul className="hidden md:flex">
            {links.map(({id, link}) => {
                return <li key={id} className="px-4 cursor-pointer capitalize font-medium text-gray-300 hover:scale-105 duration-200">
        ...
        {nav && (<ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-black to-gray-800 text-gray-300">
            {links.map(({id, link}) => {
                return <li key={id} className="px-4 cursor-pointer capitalize py-6 text-3xl">{link}</li>
            })}
        </ul>)}
